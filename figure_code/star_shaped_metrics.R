...
cp <- assess_star(cont = cp_poly, C_under = cp_C_under, C_over = cp_C_over,
                  thetas = thetas)

cm_poly <- make_poly(convex_mod, "cm")
cm_C_under <- best_C(bd = bd, conts = cm_poly, thetas = thetas,  area_tol = .001)
cm_C_over <- best_C(bd = bd, conts = cm_poly, thetas = thetas,  area_tol = .001,
                    under = FALSE)
cm <- assess_star(cont = cm_poly, C_under = cm_C_under, C_over = cm_C_over,
                  thetas = thetas)
...
circl_C_over <- best_C(bd = bd, conts = circl_poly, thetas = thetas, 
                        area_tol = .001, under = FALSE)
circl <- assess_star(cont = circl_poly, C_under = circl_C_under, 
...
curl_C_over <- best_C(bd = bd, conts = curl_poly, thetas = thetas, 
                      area_tol = .001, under = FALSE)
curl <- assess_star(cont = curl_poly, C_under = curl_C_under, 
...
mtext(text = "Under Approx.", side = 3, line = 0, at = .5, 
      outer = TRUE)
mtext(text = "Over Approx.", side = 3, line = 0, at = .82,
      outer = TRUE)
